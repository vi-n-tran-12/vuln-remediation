...
- **Parallel API calls** — session creation and polling are currently sequential (`await` one at a time). At scale with hundreds of issues, we could use `asyncio.gather()` to create/poll sessions concurrently. In practice this is low priority: each Devin API call takes <1s, and the real bottleneck is Devin's execution time (~10 min per fix), not our orchestrator's dispatch speed.
- **Dashboard pagination and search** — the current dashboard renders all tasks in a single table. At scale (hundreds of issues), this needs server-side pagination, search/filter by keyword, and sortable columns. The "Needs Input" alert banner should become a dedicated triage queue with assignee tracking so multiple engineers can divide the review work.
- **Task queue with backpressure** — currently `MAX_CONCURRENT_SESSIONS` caps parallel Devin sessions, but pending tasks are held in memory. At scale (e.g., a vulnerability scanner filing hundreds of issues at once), this needs a proper task queue (e.g., Celery + Redis, or AWS SQS) with backpressure so the system accepts work gracefully, prioritizes by severity, and dispatches to Devin at a sustainable rate without overwhelming the API or burning through session credits.
